...
describe("Blog Post Routes", () => {
	...
	describe("POST /api/posts/:postId/comments", () => {
		it("should create a comment on a post when authenticated", async () => {
			...
			expect(data.comment).toHaveProperty(
				"content",
		...
		it("should trim comment content", async () => {
			...
			expect(data.comment.content).toBe(
				"This comment should be trimmed."
			);
		});
	...
	describe("POST /api/posts/:postId/comments/:commentId/reply", () => {
		it("should create a reply to a comment when authenticated", async () => {
			...
			const response = await fetch(
				`${baseUrl}/posts/${post.id}/comments/${comment.id}/reply`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
		...
		it("should enforce thread depth limit of 5 levels", async () => {
			...
			const response = await fetch(
				`${baseUrl}/posts/${post.id}/comments/${parentComment.id}/reply`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
			...
			expect(data).toHaveProperty(
				"error",
				"Maximum thread depth of 5 levels reached"
			);
		});
	});
	...
	describe("GET /api/posts/:postId/comments", () => {
		it("should get comments for a post", async () => {
			...
			expect(data.comments[0].replies[0]).toHaveProperty(
				"content",
	...
	describe("POST /api/posts/:postId/comments/:commentId/like - Like a comment", () => {
		it("should allow authenticated user to like a comment", async () => {
			...
			const response = await fetch(
				`${baseUrl}/posts/${post.id}/comments/${comment.id}/like`,
				{
					method: "POST",
					headers: {
						Authorization: `Bearer ${authToken}`,
